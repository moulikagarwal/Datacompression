 ...
 * @author Moulik Agarwal
 * @see PriorityQueue.Position
 */
public class PairingHeap<AnyType> implements Iterable<AnyType>
{    
     ...
     */
	public PairingHeap(Comparator<AnyType> comparator) { this.comparator = comparator; }
    ...
    public void decreaseKey( Position<AnyType> pos, AnyType newVal )
    {
        ...
        if( ((Comparable<AnyType>) p.element).compareTo( newVal ) < 0 )
    ...
    private PairNode<AnyType> compareAndLink( PairNode<AnyType> first, PairNode<AnyType> second )
    {
        ...
        if( ((Comparable<AnyType>)second.element).compareTo( first.element ) < 0 )
        {
